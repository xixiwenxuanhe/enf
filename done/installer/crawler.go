...
// 修改processFile函数签名，返回FileStat
func processFile(inputFile string, maxConcurrency int) FileStat {
	start := time.Now()
...
func main() {
	...
	for _, inputFile := range inputFiles {
		fmt.Printf("\n==== 开始处理文件：%s ====\n", inputFile)
		stat := processFile(inputFile, *maxConcurrency)
		stats = append(stats, stat)
	}

	fmt.Println("\n==== 全部文件处理完毕，失败率统计如下 ====")
	for _, stat := range stats {
		fmt.Printf("文件: %s\t总记录数: %d\t失败数: %d\t失败率: %.2f%%\n", stat.FileName, stat.Total, stat.Fail, stat.FailRate)
	}
}
